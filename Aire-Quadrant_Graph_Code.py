...
#Scale the Rating Curve points.
a = -1*((0-min(height))/(max(height)-min(height))) #scaled x origin.
b = ((0-min(Flow))/(max(Flow)-min(Flow))) #scaled y origin.
def A(x):
    y = (((b-1)/(a-(-1)))*(x+1))+1
#I worked this out using the formula for the equation of a line through 2 points,
#y-y_1=m(x-x_1)
    return(y)
c = 0
d = A(c)
...
ax.plot([c,-1], [d,1], 'blue',linestyle='--', marker='', linewidth=2)
ax.plot([0,-1],[0,1],'green',linestyle='--', marker='', linewidth=2)
...
#compare the 2.
ax.fill_between(scaledday,a,b,where=a>=b,facecolor='gray')
